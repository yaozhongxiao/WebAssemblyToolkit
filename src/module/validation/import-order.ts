...
export default function validate(ast: Program): Array<string> {
  ...
  function isImportInstruction(path : NodePath<Node>) {
  ...
  traverse(ast, {
    ModuleImport(path : NodePath<Node>) {
    ...
    Global(path:NodePath<Node>) {
      if (!isImportInstruction(path)) {
        noMoreImports = true;
      }
    },
    Memory(path:NodePath<Node>) {
      if (!isImportInstruction(path)) {
        noMoreImports = true;
      }
    },
    Table(path:NodePath<Node>) {
      if (!isImportInstruction(path)) {
        noMoreImports = true;
      }
    },
