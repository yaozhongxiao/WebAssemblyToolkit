...
function equalResultTypes(t1:Array<Valtype>, t2:Array<Valtype>) {
...
export default function getType(moduleContext: ModuleContext, stack:any, instruction:any) {
  let args : Array<any> = [];
  ...
  switch (instruction.id) {
     ...
     */
    case "local": {
      instruction.args.forEach((t:any) => moduleContext.addLocal(t.name));
      args = [];
      result = [];
      break;
    }
    ...
    case "call": {
      ...
      ({ params: args , result } = moduleContext.getFunction(instruction.index.value));
    ...
    case "call_indirect": {
      ...
      args = ["i32", ...signature.params.reverse().map((p:FuncParam) => p.valtype)];
