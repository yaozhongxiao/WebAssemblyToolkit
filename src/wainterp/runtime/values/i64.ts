...
function ASSERT_NOT_ZERO(x: any): void {
  if (x._value.isZero()) {
    throw new RuntimeError("integer divide by zero");
  }
}
...
function TO_BOOLEAN(cond: any): i32 {
  return (cond) ? createTrue() : createFalse();
}
...
export class i64 implements IntegerValue<i64> {
  ...
  abs(): i64 {
    if (this._value.isNegative()) {
      // make it positive
      return new i64(this._value.mul(-1));
    }
  ...
  max(operand: i64): i64 {
    if (this._value.lessThan(operand._value) === true) {
      return operand;
    } else {
      return this;
    }
  ...
  neg(): i64 {
    return new i64(this._value.neg());
  }
