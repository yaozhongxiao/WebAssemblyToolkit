...
import { write, read } from "./ieee754-inl";

/**
 * According to https://webassembly.github.io/spec/core/binary/values.html#binary-float
 * n = 32/8
 */
...
export function decodeF32(bytes: Array<number>): number {
  const buffer = Buffer.from(bytes);

  return read(buffer, 0, true, SINGLE_PRECISION_MANTISSA, NUMBER_OF_BYTE_F32);
}

export function decodeF64(bytes: Array<number>): number {
  const buffer = Buffer.from(bytes);
