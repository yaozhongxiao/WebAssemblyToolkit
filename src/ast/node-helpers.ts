...
export function withRaw(n: NumericLiteral | Identifier, raw: string): Node {
  n.raw = raw;
  return n;
}

export function funcParam(valtype: Valtype, id: string | undefined): FuncParam {
  return {
    id,
    valtype,
  };
...
export function indexLiteral(value: number | string): Index {
  const x: NumberLiteral = numberLiteralFromRaw(value, "u32") as NumberLiteral;
  return x;
}

export function memIndexLiteral(value: number): Memidx {
  const x: U32Literal = numberLiteralFromRaw(value, "u32") as NumberLiteral;
  return x;
}
