...
export function transform(ast: Program) {
  const typeInstructions : Array<TypeInstruction> = [];

  t.traverse(ast, {
    TypeInstruction({ node } : {node : TypeInstruction}) {
  ...
  function denormalizeSignature(
    ...
    if (signature.type === "Identifier") {
      ...
      const typeInstruction = typeInstructions.find(
        (t: TypeInstruction) => t.id!.type === identifier.type && t.id!.value === identifier.value
      );
