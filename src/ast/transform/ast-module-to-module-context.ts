...
export function moduleContextFromModuleAST(m: Module): ModuleContext {
  ...
  m.fields.forEach((field) => {
    switch (field.type) {
      ...
      case "ModuleImport": {
        const import_descr_type = field.descr.type;
          ...
          default:
            throw new Error(
              "Unsupported ModuleImport of type " +
                JSON.stringify(import_descr_type)
            );
        }
        break;
      }
...
export class ModuleContext {
  ...
  addFunction(func: Func) {
    ...
    let { params: args = [], results: result = [] } = func.signature as Signature || {};
    /* eslint-enable */

    const params = args.map((arg) => arg.valtype);

    this.funcs.push({ params, result });
  ...
  importFunction(funcimport: any) {
    if (isSignature(funcimport.signature)) {
      ...
      let { params: args, results: result } = funcimport.signature as Signature;
      let params:Array<Valtype> = args.map((arg) => arg.valtype);
      ...
      const signature : Signature= this.getType(typeId);
      this.funcs.push({
  ...
  getFunctionOffsetByIdentifier(name: string): any {
